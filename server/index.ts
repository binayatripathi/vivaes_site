  ...
  try {
    ...
    if (replitDomains) {
      ...
      try {
        ...
        console.warn('Webhook setup skipped (will work after deploy):', (webhookErr as Error).message);
      }
    } else {
...
app.use((req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    if (path.startsWith("/api")) {
      ...
      if (capturedJsonResponse && !path.startsWith("/api/admin") && !path.startsWith("/api/vapi")) {
